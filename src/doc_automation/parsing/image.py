...
def rasterize_page(path: Path, page_num: int, dpi: int = _DEFAULT_DPI) -> Any:
...
def extract_image_pdf(path: Path, dpi: int = _DEFAULT_DPI) -> ParsedDocument:
    ...
    for page_num in range(page_count):
        ...
        words, page_text = ocr_image(image, page_num=page_num)
...
def parse_image_file(path: Path) -> ParsedDocument:
    ...
    image = preprocess(image)
    words, page_text = ocr_image(image, page_num=0)
    return ParsedDocument(
        path=path,
        page_count=1,
